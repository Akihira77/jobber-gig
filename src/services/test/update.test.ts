...
import { getGigById, updateActiveGigProp, updateGig, updateGigReview } from "../gig.service";
...
describe("Update Gig", () => {
    ...
    afterAll(async () => {
        await updateGig(gigId, {
            ...
            description: "Ventito sordeo copiose teres aedificium compello sopor demo. Mollitia argentum careo angelus aperiam vergo quis utor.",
            basicTitle: "Elegant Rubber Keyboard",
            basicDescription: "Carbonite web goalkeeper gloves are ergonomically designed to give easy fit",
            ...
            ratingsCount: 2,
            ratingSum: 10,
            ratingCategories: { "five": { "value": 0, "count": 0 }, "four": { "value": 0, "count": 0 }, "three": { "value": 0, "count": 0 }, "two": { "value": 0, "count": 0 }, "one": { "value": 0, "count": 0 } },
            price: 30,
    ...
    describe("updateGig() method", () => {
        it("should be saved to mongodb and elasticsearch and they must be exactly same", async () => {
            ...
            await updateGig(gigId, reqBody);

            const result = await getGigById(gigId)
        ...
        it("incorrect gigId - should return null", async () => {
            await expect(updateGig("65e9e422a18101bb1b1b595p", {} as ISellerGig)).rejects.toThrow("Invalid gig id");
        })
    })

    describe("updateActiveGigProp() method", () => {
        it("should be saved to mongodb and elasticsearch and they must be exactly same 1", async () => {
            const result = await updateActiveGigProp(gigId, false);
        ...
        it("should be saved to mongodb and elasticsearch and they must be exactly same 2", async () => {
            const result = await updateActiveGigProp(gigId, true);
            const gig = await getGigById(gigId);
        ...
        it("incorrect gigId - should return null", async () => {
            await expect(updateActiveGigProp("65e9e422a18101bb1b1b595p", true)).rejects.toThrow("Invalid gig id");
        })
    })

    describe("updateGigReview() method", () => {
        it("success case", async () => {
            const prevGig = await getGigById(gigId);
            await updateGigReview({ gigId, type: "buyer-review", rating: 5, sellerId: "65e9cc021d2eacf8631ba6da" })
            const currGig = await getGigById(gigId);
        ...
        it("incorrect gigId - should return null", async () => {
            await expect(updateGigReview({ gigId: "65e9e422a18101bb1b1b595p", type: "buyer-review", rating: 5 })).rejects.toThrow("Invalid gig id");
        })
    })
})
