...
export class GigQueue {
	...
	async consumeGigDirectMessages(): Promise<void> {
		try {
			...
			await this.ch.assertExchange(gigService.updateGig.exchangeName, "direct");
			...
			this.logger("queues/gig.consumer.ts - consumeGigDirectMessages()").error(
	...
	async consumeSeedDirectMessages(): Promise<void> {
		try {
			...
			await this.ch.assertExchange(gigService.seed.exchangeName, "direct");
			...
			this.logger("queues/gig.consumer.ts - consumeSeedDirectMessages()").error(
	...
	async publishDirectMessage(
		...
		try {
			...
			this.logger("queues/gig.producer.ts - publishDirectMessage()").error(
